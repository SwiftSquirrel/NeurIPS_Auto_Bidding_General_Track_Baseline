...
class IQL(nn.Module):
    ...
    def calc_policy_loss(self, states, actions):
        ...
        a_ub = torch.FloatTensor([100.0])
        if torch.cuda.is_available():
            a_ub = a_ub.to('cuda')
        exp_a = torch.exp(min_Q - v) * self.temperature
        exp_a = torch.min(exp_a, a_ub)
    ...
    def update_target(self, local_model, target_model):
        for target_param, local_param in zip(target_model.parameters(), local_model.parameters()):
            target_param.data.copy_(
                (1. - self.tau) * target_param.data + self.tau * local_param.data)
    ...
    def load_net(self, load_path="saved_model/fixed_initial_budget", device='cuda:0'):
        ...
        if os.path.isfile(load_path + "/critic.pt"):
            self.critic1.load_state_dict(torch.load(
                load_path + "/critic1.pt", map_location='cpu'))
            self.critic2.load_state_dict(torch.load(
                load_path + "/critic2.pt", map_location='cpu'))
            self.actors.load_state_dict(torch.load(
                load_path + "/actor.pt", map_location='cpu'))
        else:
            self.critic1 = torch.load(
                load_path + "/critic1.pkl", map_location='cpu')
            self.critic2 = torch.load(
                load_path + "/critic2.pkl", map_location='cpu')
            self.actors = torch.load(
                load_path + "/actor.pkl", map_location='cpu')
        self.value_net = torch.load(
            load_path + "/value_net.pkl", map_location='cpu')
