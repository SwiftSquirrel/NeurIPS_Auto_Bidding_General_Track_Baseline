...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        if torch.cuda.is_available():
            return states.to('cuda'), actions.to('cuda'), rewards.to('cuda'), next_states.to('cuda'), dones.to('cuda')
        else:
...
if __name__ == '__main__':
    buffer = ReplayBuffer()
    for i in range(1000):
        buffer.push(np.array([1, 2, 3]), np.array(
            4), np.array(5), np.array([6, 7, 8]), np.array(0))
