...
class BcBiddingStrategy(BaseBiddingStrategy):
    ...
    def bidding(self, timeStepIndex, pValues, pValueSigmas, historyPValueInfo, historyBid,
        ...
        for auction_result, impression_result in zip(historyAuctionResult, historyImpressionResult):
            history_realCost.append(
                auction_result[:, 0]*impression_result[:, 0])
            history_conversion.append(impression_result[:, 1])
            history_realCost_mean.append(
                np.mean(auction_result[:, 0]*impression_result[:, 0]))
        ...
        historical_volume = sum(len(bids)
                                for bids in historyBid) if historyBid else 0
        ...
        # bid / leatWinningCost
        historical_bid_mp_ratio = historical_bid_mean / \
            (historical_LeastWinningCost_mean + 1e-8)
        last_three_bid_mp_ratio = last_three_bid_mean / \
            (last_three_LeastWinningCost_mean + 1e-8)
        ...
        for key, value in self.normalize_dict.items():
            test_state[key] = normalize(
                test_state[key], value["min"], value["max"])
